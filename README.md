...
- [Kimi](docs/kimi.md) — Auth token (JWT from `kimi-auth` cookie) for weekly quota + 5‑hour rate limit. Manual mode
  avoids browser Safe Storage prompts by using a pasted `kimi-auth` token or cookie header directly.
